...
document.addEventListener("DOMContentLoaded", () => { 
  ...
  Promise.all([getUserInfo(), getInitialCards()])
    .then(([userData, cards]) => { 
      ...
      cards.forEach((card) => { 
        placesList.append(createCard(card, openImagePopup, currentUserId)); 
      }); 
    }) 
    .catch((err) => 
  ...
  profileForm.addEventListener("submit", (e) => { 
    ...
    updateUserInfo(nameInput.value, descInput.value) 
      ...
      .finally(() => renderLoading(profileForm, false)); 
  }); 

  postForm.addEventListener("submit", (e) => { 
    e.preventDefault(); 
    renderLoading(postForm, true); 

    addCard(postNameInput.value, postUrlInput.value) 
      .then((newCard) => { 
        placesList.append(createCard(newCard, openImagePopup, currentUserId)); 
        closePopup(popupAddCard); 
      }) 
      .catch(() => console.error("Ошибка добавления карточки")) 
      .finally(() => renderLoading(postForm, false)); 
  }); 
